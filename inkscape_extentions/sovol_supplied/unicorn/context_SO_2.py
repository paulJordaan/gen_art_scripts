...
class GCodeContext:
    def __init__(self, xy_feedrate, z_feedrate, start_delay, stop_delay, pen_up_angle, pen_down_angle, z_height, finished_height, x_home, y_home, register_pen, num_pages, continuous, file):
      ...
      if False:
        ...
        self.registration = [
          "M400 (finish all movement)",
          "M280 P0 S%d (pen down)" % (self.pen_down_angle),
          "G4 P%d (wait %dms)" % (self.start_delay, self.start_delay),
          "M280 P0 S%d (pen up)" % (self.pen_up_angle),
          "G4 P%d (wait %dms)" % (self.stop_delay, self.stop_delay),
        ...
        self.postscript = [
          "",
          "G1 Z%.2f F%.2f" % (self.pen_up_angle, self.z_feedrate),
        ...
        self.registration = [
          "G1 Z%.2f F%.2f" % (self.pen_down_angle, self.z_feedrate),
          "G1 Z%.2f F%.2f" % (self.pen_up_angle, self.z_feedrate),
          ""
        ]
        self.sheet_header = [
    ...
    def start(self):
      ...
      self.codes.append("G1 Z%.2f F%.2f" % (self.pen_down_angle, self.z_feedrate))
      # self.codes.append("G4 P%d " % (self.start_delay))
      self.drawing = True

    def stop(self):
      # self.codes.append("M400 (finish all movement)")
      self.codes.append("G1 Z%.2f F%.2f" % (self.pen_up_angle, self.z_feedrate))
    ...
    def draw_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing == False:
            # self.codes.append("M400 (finish all movement)")
            self.codes.append("M280 P0 S%0.2F (pen down)" % self.pen_up_angle)
            self.codes.append("G4 P%d (wait %dms)" % (self.start_delay, self.start_delay))
            self.drawing = True
